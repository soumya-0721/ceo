...
async function renderDashboard() {
    ...
    try {
        ...
        if (isFocus)
            statusHtml = '<span class="status-indicator status-focus-mode"><span class="status-dot"></span>Focus Mode</span>';
        else if (currentMeeting)
            statusHtml = '<span class="status-indicator status-meeting"><span class="status-dot"></span>In Meeting</span>';
...
function exportBookingsToExcel() {
    api.exportBookingsExcel().then((bookings) => {
        if (!bookings.length) {
...
function openReminderModal(reminder) {
    const modal = new window.bootstrap.Modal(document.getElementById('reminderModal'));
...
function openFocusModal() {
    const modal = new window.bootstrap.Modal(document.getElementById('focusModal'));
    document.getElementById('foc-date').value = new Date().toISOString().split('T')[0];
    modal.show();
}
function openFindTimeModal() {
    const modal = new window.bootstrap.Modal(document.getElementById('findTimeModal'));
...
async function findAvailableSlots() {
    ...
    if (!date) {
        el.innerHTML = '<div class="alert alert-warning">Please select a date</div>';
        return;
    }
    try {
