...
async function migrate() {
    try {
        ...
        const users = await database_1.default.query("SELECT username, full_name FROM users");
        ...
        if (users.rows.length === 0) {
            ...
            await database_1.default.query("INSERT INTO users (username, password_hash, full_name, email, role) VALUES ($1,$2,$3,$4,$5)", ['ceo', hash, 'Samhith', 'samhith@next360.com', 'ceo']);
            await database_1.default.query("INSERT INTO users (username, password_hash, full_name, email, role) VALUES ($1,$2,$3,$4,$5)", ['soumya', hash2, 'Soumya', 'soumya@next360.com', 'coordinator']);
            ...
            const hash = await bcrypt.hash('ceo123', 10);
            const hash2 = await bcrypt.hash('soumya123', 10);
            await database_1.default.query("UPDATE users SET password_hash=$1 WHERE username='ceo'", [hash]);
            await database_1.default.query("UPDATE users SET password_hash=$1 WHERE username='soumya'", [hash2]);
